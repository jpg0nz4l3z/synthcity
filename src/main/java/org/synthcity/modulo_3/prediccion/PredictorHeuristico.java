...
public class PredictorHeuristico implements Predictor {
    ...
    private TendenciaPredicha determinarTendencia(PredictionInput input, double score) {
        ...
        if (input.getPorcentajeActivos() >= UMBRAL_ACTIVIDAD_BALANCE
            ...
            return score >= 70.0 ? TendenciaPredicha.MEJORA_PROBABLE : TendenciaPredicha.ESTABLE;
        }
    ...
    private double calcularConfianza(PredictionInput input, TendenciaPredicha tendencia) {
        if (tendencia == TendenciaPredicha.SIN_BASE) return 0.20;

        double confianza = 0.60;
        if (input.getTotalBloques()         >= 10) confianza += 0.10;
        if (input.getConteoPorTipo().size() >= 3)  confianza += 0.10;
        if (input.getEstabilidadBasica()    >= 0.60) confianza += 0.10;
    ...
    private String generarMensaje(TendenciaPredicha tendencia, double score) {
        return switch (tendencia) {
            ...
            case ESTABLE ->
                    "La ciudad presenta una proyección estable con score "
