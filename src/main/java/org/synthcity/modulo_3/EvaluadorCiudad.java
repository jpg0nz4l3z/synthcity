...
public class EvaluadorCiudad implements Evaluable {
    ...
    public EvaluadorCiudad() {
        this(null, new GestorExpansion(), new ConstructorDataset(), new PredictorHeuristico());
    }


    public EvaluadorCiudad(Simulable simulable) {
        this(simulable, new GestorExpansion(), new ConstructorDataset(), new PredictorHeuristico());
    }
    ...
    public EvaluadorCiudad(Simulable simulable,
        ...
        this(simulable, gestorExpansion, constructorDataset, new PredictorHeuristico());
    }
    ...
    public EvaluadorCiudad(Simulable simulable,
        ...
        if (gestorExpansion == null || constructorDataset == null) {
            throw new ResultadoSimulacionInvalidoException(
                    "Las dependencias del evaluador no pueden ser nulas.");
        }
        if (predictor == null) {
            throw new ResultadoSimulacionInvalidoException(
                    "El predictor no puede ser nulo.");
        }
    ...
    private void validarEntrada(ResultadoSimulacion resultado) {
        if (resultado == null) {
            throw new ResultadoSimulacionInvalidoException(
                    "El resultado de simulación no puede ser nulo.");
        }
        if (resultado.getBloquesTotales() < 0
                || resultado.getBloquesActivos() < 0
                || resultado.getBloquesInactivos() < 0) {
            throw new ResultadoSimulacionInvalidoException(
                    "Los contadores de bloques no pueden ser negativos.");
        }
        if (resultado.getBloquesActivos() > resultado.getBloquesTotales()
                || resultado.getBloquesInactivos() > resultado.getBloquesTotales()) {
            throw new ResultadoSimulacionInvalidoException(
                    "Los bloques activos o inactivos no pueden superar el total.");
        }
        if ((resultado.getBloquesActivos() + resultado.getBloquesInactivos())
                != resultado.getBloquesTotales()) {
            throw new ResultadoSimulacionInvalidoException(
                    "Incoherencia: Activos + Inactivos no suma el total.");
        }
    ...
    private double calcularScoreViabilidad(MetricaCiudad m) {
        if (m.getTotalBloques() <= 0) return 0.0;
    ...
    private NivelEvaluacion determinarNivelEvaluacion(
        ...
        if (score >= UMBRAL_SCORE_FUNCIONAL) return NivelEvaluacion.FUNCIONAL;
        if (score >= UMBRAL_SCORE_INESTABLE) return NivelEvaluacion.INESTABLE;

        return NivelEvaluacion.CRITICO;
    }

    private String generarMensaje(NivelEvaluacion nivel, MetricaCiudad m,
                                  double score, Set<AlertaEvaluacion> alertas) {
    ...
    private String generarResumenRiesgo(Set<AlertaEvaluacion> alertas) {
        if (alertas.isEmpty()) return "Sin riesgos relevantes detectados.";
    ...
    private TendenciaTemporal determinarTendenciaTemporal(MetricaCiudad m) {
        if (m.getTotalBloques() <= 0 || m.getCiclosEjecutados() <= 0)
            return TendenciaTemporal.SIN_DATOS;
        if (m.colapsoDetectado())
            return TendenciaTemporal.COLAPSANDO;
        if (m.saturacionDetectada() || m.isNecesidadExpansionDetectada())
            return TendenciaTemporal.SATURANDO;
        if (m.estaEnTendenciaNegativa() || m.tieneContaminacionCreciente())
            return TendenciaTemporal.DETERIORANDO;
        if (m.estaEnTendenciaPositiva())
            return TendenciaTemporal.MEJORANDO;
